...
def gen_proxy(proxies):
    ...
    '''
    a = proxies[np.random.randint(5, 80)]
    return {a[:a.find(":")]: a}
...
def getWeb(url, Return, proxies, timeout=1, Max=20, sleep=True, sleepMultiply=1):
    '''
    ...
    while a:
        i += 1
        try: response = requests.get(url, proxies=proxy, timeout=timeout); a = response.status_code!=200; print("第{}次代理成功".format(i))
        ...
        if sleep: time.sleep(np.random.rand()*sleepMultiply)
    ...
    elif Return == "": return response
    
def get_start_end_date(releaseDate, delta, sep=False):
    '''
    时间转换：给定发行日期和前后间隔天数，计算开始日期和结束日期。
